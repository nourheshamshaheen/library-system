...
public class adminLogin {
	...
	private Admin[] all_users = new Admin[100];
	...
	public void login()
	{
		...
		label_title = new JLabel("Admin Login");
		label_title.setBounds(100,20,150,50);
	...
	public int getDatabase_admin()
	{
		int i = 0;
		try{ 
			File login_database = new File("Admins.txt");
			Scanner stream = new Scanner(login_database);
			
			while(stream.hasNext()) {
				
				all_users[i] = new Admin(stream.next(), stream.next());
				stream.nextLine();
				i++;
			}
			stream.close();
