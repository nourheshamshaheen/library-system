...
import javax.swing.JTextField;
		...
		public class studentLogin {
			...
			private Student[] all_users = new Student[100];
			...
			public void login()
			{
				...
				label_title = new JLabel("Student Login");
			...
			public int getDatabase()
			{
				int i = 0;
				try{ 
					File login_database = new File("Students.txt");
					Scanner stream = new Scanner(login_database);
					stream.useDelimiter(",");
					while(stream.hasNextLine()) {
						
						all_users[i] = new Student(stream.next(), stream.next());
						stream.nextLine();
						i++;
					}
					stream.close();
			...
			public void login_actionPerformed(ActionEvent e) {
				...
				else
				{
					...
					if(success)
					{
						...
						frame.dispose();						
						new studentSection(user, passwordstr);
					}
					else if(!success)
					{
